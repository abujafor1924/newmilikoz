...
class BookListAPIView(APIView):
    ...
    @swagger_auto_schema(
        operation_summary="All Product List", responses={200: BookSerializer(many=True)}
    )
        ...
        return paginator.get_paginated_response(
            {
                ...
                "metadata": {
                    ...
                    "current_page": (
                        paginator.page.number if hasattr(paginator, "page") else 1
                    ),
                },
            }
        )
    ...
    @swagger_auto_schema(
        operation_summary="Create an New Post",
...
class BookDetailAPIView(APIView):
    ...
    @swagger_auto_schema(
        operation_summary="Product Deatils View", responses={200: BookSerializer}
    )
    ...
    @swagger_auto_schema(
        operation_summary="Product Details View", responses={200: BookSerializer}
    )
    def get(self, request, pk):
        ...
        try:
            ...
            return Response(
                {"success": False, "message": f"Book with id {pk} not found"},
                status=status.HTTP_404_NOT_FOUND,
            )

    @swagger_auto_schema(
        operation_summary="Update  Details",
        request_body=BookSerializer,
        responses={201: BookSerializer},
    )
    ...
    @swagger_auto_schema(
        ...
        responses={
            200: BookSerializer,
            404: "Book not found",
            400: "Cannot delete unavailable book",
        },
    )
...
class BookOperationsView(APIView):
    ...
    def bulk_delete(self, request):
        ...
        if not book_ids:
            return Response(
                {"success": False, "message": "No book IDs provided"},
                status=status.HTTP_400_BAD_REQUEST,
            )
            ...
            return Response(
                {
                    ...
                    "old_price": (
                        float(original_price) if "original_price" in locals() else None
                    ),
