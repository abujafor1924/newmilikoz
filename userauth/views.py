...
from .serializers import (LoginSerializer, PasswordResetConfirmSerializer,
                          PasswordResetRequestSerializer, ResetOtpSerializer,
...
class RequistPasswordResetAPIView(generics.GenericAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        user_instance.save(
            update_fields=["password_reset_otp", "password_reset_otp_created_at"]
        )
    ...
    def post(self, request):
        ...
        try:
            ...
            if not user.password_reset_token:
                return Response(
                    {
                        "error": "Password reset token is invalid or has not been generated."
                    },
                    status=status.HTTP_400_BAD_REQUEST,
                )
